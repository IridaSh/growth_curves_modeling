...
class CNNDecoder(nn.Module):
    ...
    def __init__(self, latent_dim, latent_channel, seq_length):
        ...
        self.decoder = nn.Sequential(
            nn.ReLU(),
            nn.ConvTranspose1d(latent_channel, 16, kernel_size=3, stride=1, padding=1, output_padding=0),
            nn.ReLU(),
            nn.ConvTranspose1d(16, 64, kernel_size=3, stride=1, padding=1, output_padding=0),
            nn.ReLU(),
            nn.ConvTranspose1d(64, 128, kernel_size=3, stride=1, padding=1, output_padding=0),
            nn.ReLU(),
            nn.ConvTranspose1d(128, 1, kernel_size=3, stride=1, padding=1, output_padding=0),
        )
...
if __name__ == '__main__':
    ...
    vae_model.load_state_dict(torch.load('/hpc/group/youlab/is178/vae_model_2.pt'))
